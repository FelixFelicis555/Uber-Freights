package com.uber.lastmile.client;
...
public class GoogleMapsClient {
    public static RouteOptionResponse rewardsAndTimeFiller(RouteOptionResponse routeOptionResponse){
        String url ="https://maps.googleapis.com/maps/api/distancematrix/json?origins="+routeOptionResponse.getPackageSourceX()+","+routeOptionResponse.getPackageSourceY()+"&destinations="+routeOptionResponse.getPackageDestinationX()+","+routeOptionResponse.getPackageDestinationY()+"&key="+Constants.MAPS_API_KEY;
      ...
      try{
        ...
        int reward=(int)Math.round(element.get("distance").get("value").asInt()*Constants.REWARDS_PER_METER);
